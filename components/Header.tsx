...
export function Header() {
  ...
  return (
    <>
      <header 
        className={`fixed top-0 left-0 right-0 z-50 bg-background/90 backdrop-blur-xl border-b border-border/20 transition-all duration-500 ${
          isVisible ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"
        }`}
      >
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="apple-hover">
            <h1 className="text-xl tracking-tight font-light">Slideintoo</h1>
          </div>
          <div>
            <Button 
              size="sm" 
